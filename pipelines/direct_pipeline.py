...
class DirectPipeline:
    ...
    Reads audio from a local file, streams PCM chunks directly
    to an STT provider, and evaluates the transcription."""
    ...
    async def run_sample(self, sample: Sample) -> BenchmarkResult:
        transport = DirectTransport(sample.audio, chunk_ms=self.chunk_ms)
    ...
    async def run_dataset(self, samples: list[Sample]) -> list[BenchmarkResult]:
        ...
        for sample in samples:
            logger.info("Direct pipeline: %s", sample.id)
            try:
