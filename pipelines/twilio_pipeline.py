...
class TwilioPipeline:
    """Benchmark STT through the Twilio Media Streams path.

    Audio arrives via WebSocket -> mu-law decode -> resample ->
    PCM chunks -> STT Provider -> evaluation."""
    ...
    def __init__(
        ...
        self.transport = transport or TwilioMediaTransport()

    async def run_from_buffer(self, sample: Sample) -> BenchmarkResult:
        """Run after audio has been buffered into the transport."""
